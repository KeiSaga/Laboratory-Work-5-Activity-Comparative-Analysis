...
> ⚠️ Early Stopping triggered at Epoch 3. The model was actively learning (train acc: 18% → 54%, val acc: 45% → 71%) but `patience=3` halted it too early. With more epochs, this model is expected to reach 88–93%.
...
> ⚠️ Early Stopping triggered at Epoch 3. ResNet50 showed the strongest per-epoch learning gain (val acc jumped from 46% → 77% in 3 epochs) — stopping too early significantly limited final performance. Expected val accuracy with 10 full epochs: 85–92%.
...
>
> ⚠️ **Note:** EfficientNetB0 and ResNet50 were stopped too early by `patience=3`. Their training curves show consistent improvement — increasing patience to 5 in a re-run would allow full convergence.
...
|---|---|---|
| MobileNetV2 | `Conv_1` | ✅ Activation focused on the target object |
...
**2. EfficientNetB0 and ResNet50 were stopped too early** — both models showed strong learning trajectories (EfficientNetB0: 45% → 71% val acc; ResNet50: 46% → 77% val acc in just 3 epochs). The `patience=3` callback was too aggressive for these deeper architectures.
...
**4. Speed comparison** — MobileNetV2 ran at 2s/step, EfficientNetB0 at 3s/step, ResNet50 at 7s/step. For balanced speed and accuracy, MobileNetV2 is the recommended deployment choice.
...
├── README.md                               # This file
└── screenshots/
    ├── MobileNetV2_training_curves.png     # Model 1 — training history
    ├── MobileNetV2_confusion_matrix.png    # Model 1 — confusion matrix
    ├── MobileNetV2_roc_curve.png           # Model 1 — ROC curve
    ├── EfficientNetB0_training_curves.png  # Model 2 — training history
    ├── EfficientNetB0_confusion_matrix.png # Model 2 — confusion matrix
    ├── EfficientNetB0_roc_curve.png        # Model 2 — ROC curve
    ├── ResNet50_training_curves.png        # Model 3 — training history
    ├── ResNet50_confusion_matrix.png       # Model 3 — confusion matrix
    ├── ResNet50_roc_curve.png              # Model 3 — ROC curve
